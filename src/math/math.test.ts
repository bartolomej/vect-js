...
describe('Test matrix class', function () {
  ...
  it('should calculate 2x2 matrix determinate', function () {
    let m = new Matrix([[2, 1], [0, 3]]);
    expect(m.determinate2D()).toBe(6);
  });

  it('should calculate 2x2 matrix inverse', function () {
    let m = new Matrix([[4, 2], [7, 6]]);
    expect(m.inverse2D()).toEqual(new Matrix([[0.6000000000000001, -0.2], [-0.7000000000000001, 0.4]]))
  });
